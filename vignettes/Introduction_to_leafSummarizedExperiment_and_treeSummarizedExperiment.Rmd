...
The `leafSummarizedExperiment` and `treeSummarizedExperiment` class are both
extended from `SummarizedExperiemnt` class. They are used to store rectangular
data of experimental results as `SummarizedExperiemnt`, and also support the
storage of a hiearchical structure and its link information to the rectangular
data .

# leafSummarizedExperiment
## Compare to SummarizedExperiment
...
`metadata`. More details about the structure are in [SummarizedExperiment for
coordinating experimental assays, samples, and regions of
...
* It requires a `phylo` object stored in `metadata` and named as `tree`.
* It requires the `phylo` object has unique label for each leaf node.
* It requires a column `nodeLab` in `rowData`, or row names of `rowData` to
provide the label of a node that a row is mapped to.

Here, we use toy data as an example to show the difference between using
`SummarizedExperiment` and `leafSummarizedExperiment`.
Assume that *toyTable* is observations of 5 entities collected from 4 samples.
```{r}
...
Descripitons about `r nrow(toyTable)` entities and `r ncol(toyTable)`
...
The hierarchical structure of the 10 entities is *toyTree*. It's a `phylo`
...
To store data above, we could use `SummarizedExperiment` as below.
```{r}
...
As shown, *se* could be created successfully without checking whether rows of
...
structure is ignored. We hence provide a function `leafSummarizedExperiment` to
fill this gap. 

The map between the row of *toyCount* and the leaf node of *toyTree* is checked
when creating a `leafSummarizedExperiment` object. If the same data above is
used, we get a message that 5 rows of *toyTable* are removed due to the mismatch
with the node label of the tree. The `assays` table of *test* has no rows.
```{r}
...
between the rows of *toyCount* and the tree *toyTree* via a column `nodeLab` in
`rowData` or via the row names of `rowData`.

Create a `leafSummarizedExperiment` object by changing the row names of `rowData`.
```{r eval=FALSE}
...
test_1 <- leafSummarizedExperiment(assays = list(toyTable_1), rowData = rowInf_1,
                           colData = colInf, tree = tinyTree)
assays(test_1)[[1]]
```

Create a `leafSummarizedExperiment` object by adding a `rowData`.
```{r}
...
to do it by adding a column `nodeLab` in the `rowData` because it keeps the
...
The `leafSummarizedExperiment` only support data on the level of leaf node. If
users have some rows mapped to internal nodes of a tree, then
`treeSummarizedExperiment` should be used. Technically, the
...
Here, we construct a `treeSummarizedExperiment` object using data from previous
section as a toy example. With the function `nodeValue`, data at all nodes of
...
its descendant nodes. Here, the sum is used. To view the running process,
`message = TRUE` is used.
...
`treeSummarizedExperiment` is an extension from `SummarizedExperiment` class.
```{r}
class(tse)
...
use `assays` function. This is similar to `SummarizedExperiment` class. 
...
those at its descendant leaves.  More details about how to use r
Biocpkg("ggtree") to plot tree could be seen
...
The column data and metadata could be accessed exactly the same as those in the
...
in `assays`, and they are exactly in the same order. The column *rowID* is about
...
If the label of internal nodes are available and unique on the tree, the link
...
(orange texts) for internal nodes.

### Tree data

The data about the tree structure could be accessed using `treeData()`. It is a
`phylo` object. 
```{r}
treeD <- treeData(tse)
...
node label is given in orange texts, and the node number is in blue.
...
If the tree provided has unique label for all nodes, we could also construct a
...
# Reference
Have difficulty to cite SummarirzedExperiment package....
