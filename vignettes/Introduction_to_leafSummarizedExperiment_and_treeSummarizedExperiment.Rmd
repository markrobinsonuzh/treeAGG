...
be displayed. See *new_tse* in Section 3.1.3 of the vignette *Example of data
analysis* as an example. The result tables obtained from `r Biocpkg("edgeR")`
are named as `result_assay1` and stored as a column of the `rowData`. It's the
internal part of the row data and is visible with `internal = TRUE`. The
internal data is generated by functions, such as, `runEdgeR` or `updateTSE`, and
is designed to store the analysis results of the tables from `assays`. Users are
not allowed to change them. Here, as *tse* doesn't have internal row data, we
