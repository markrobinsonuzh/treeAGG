...
count <- nodeCount(data = cytofCount, 
                   tree = pTree,
                   fun = sum)
dim(count)
```
...
The Bioconductor package `r Biocpkg("edgeR")` is used here to investigate whether an entity shows differential abundance between conditions, but users are free to perform the testing using any suitable software. The only requirement is that the differential testing outputs a multiple testing-adjusted p-value at each tree node, which can be further used to do the aggregation. 
