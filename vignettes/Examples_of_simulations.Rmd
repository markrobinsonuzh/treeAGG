...
We would like to simulate a count table, where some rows have differental
abundance between samples with different phenotypic outcomes. The easy way to do
it is to randomly select some rows and increase or decrease the counts of
samples with certain phenotypic outcomes. However, this might cause issues in
compositional data because the proportion of a row might change even though its
count has not changed. To avoid such issues, instead of adding counts to
specific rows, we swap abundances between groups of rows.
...
distribution as has been suggested in several articles [@Zhao2015,@Tang2016a,@Tang2016b,@Wu2016]:
...
In the `r CRANpkg("dirmult")` package, the Dirichlet Multinomial distribution (equation
\@ref(eq:dirmult1)) is reparameterized with $\mathbf{\pi} = (\pi_1, ..., \pi_K)$
...
3 simulation scenarios (S1, S2, and S3), which differ in how this swap is
performed.

+ S1 (the left plot in Figure \@ref(fig:scenarioDiff)): The proportions of
entities in sample $i$ are $\mathbf{p^A_{i}}$ and
...
Compared with group A, the proportions of entities on the blue and orange branch
...
Compared with group A, the proportions of entities on the colored branches have
...
differentially abundant in S3. Technically, we could not say those entities with blue
...
$$\text{p}^A_{i} \sim \text{Dirichlet}(\bf{\hat{\pi}}, \hat{\theta}).$$
$$\text{p}^B_{i} \sim \text{Dirichlet}(\hat{\pi}', \hat{\theta})$$
Compared with group A, the proportions of entities on the orange branches have
...
entities. In S3, the swap could be partial and depends on the value $c$.
...
has the samples in the rows and entities in the columns, so we need to transpose
...
`SummarizedExperiment` class that additionally contains a `phylo` object (tree)
...
function `parEstimate` is used to do the estimation once and save results in the
...
rb
```
The proportion of branch A is `r rb[2, "A_prop(%)"]`$\%$ in `dat3`. This
fulfills the requirement that the relative abundance level of branch A isn't
...
value in formula \@ref(eq:s3) and could be chosen between 0 and 1. If
it's set as NULL, the swap is similar to S1 that the proportions of the selected
branches are swapped. Note the blue branch here is not the branch specified by
