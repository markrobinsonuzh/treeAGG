...
The output of the data analysis is stored in the internal part of the `rowData`
that is only visible when using `rowData` with `internal = TRUE`. We don't
recommend users to modify the internal part of the `rowData` because it's
prepared for the aggregation step.
```{r}
# This extracts the original row data
rD1 <- rowData(new_tse, internal = FALSE)
colnames(rD1)

# The result is store in the column result_assay1
rD2 <- rowData(new_tse, internal = TRUE)
colnames(rD2)
...
points. Nodes with orange points are the suggested as the optimal level at which
to interpret the signal. More details about how to use `r Biocpkg("ggtree")`
could be seen
...
Compared to the original row data, there are two new columns, *result_assay1*
and *result_assay2*. The former stores the output from the first element of the
`assays`, and the later stores the second one.
```{r}
# the new row data
rD3 <- rowData(new_tseM, internal = TRUE)
colnames(rD3)

# show new columns
setdiff(colnames(rD3), colnames(rD1))
```
...
# name the results as the list of the contrasts
...
Now, the output could be found in the internal part of the `rowData`.
```{r}
# the new row data
rD4 <- rowData(test_tseM, internal = TRUE)

# two new columns created
colnames(rD4)

# Result gained from each contrast is stored as a column
colnames(rD4$result_assay1)
```
